...
function [xout2,yout2,Pout2, Mout2, Dout2, xavgout,yavgout] = plotschnitzme_2019_06_12_no_ploy_5(schnitzcells, xfield, yfield, whichones, style, varargin)
...
for i = 1:length(whichones)
    ...
    while ~done, 
        x = schnitzcells(thisone).(xfield);
        y = schnitzcells(thisone).(yfield);
        pa = schnitzcells(thisone).P;
        e = schnitzcells(thisone).E;
        m = schnitzcells(thisone).D;
...
%         M = cat(2,M,m);
    ...
    %    hout(i)=plot(X2,Y2,style); hold on;
            ...
            yout2(X2,i)=Y2;
            xout{i} = X2;
            xout2(1:length(X2),i)=X2;
            
            Pout2(1:length(Pa),i)=Pa(end:-1:1);
            Mout2(1:length(M),i)=M(end:-1:1);
            Dout2(1:length(E),i)=E(end:-1:1);
