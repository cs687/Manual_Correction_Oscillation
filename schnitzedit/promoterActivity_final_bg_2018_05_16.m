function snew = promoterActivityPoly(s, gamma_fudge, back,backr);
...
%        MC = [s(P).MC((end - appendedFramesParent + 1) : end) MC];
        ...
        MR = [s(P).MR((end - appendedFramesParent + 1) : end)-backr MR];
    else
        appendedFramesParent = 0;
    end
...
%        MC = [MC (s(D1).MC(1 : appendedFramesChildren) + s(D2).MC(1 : appendedFramesChildren))/2];
        ...
        MR = [MR (s(D1).MR(1 : appendedFramesChildren)-backr + s(D2).MR(1 : appendedFramesChildren))/2];
    else
